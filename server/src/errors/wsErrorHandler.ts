...
const wsErrorHandler = (error: unknown, ws: WebSocket, action: unknown) => {
  ...
  try {
    if (error instanceof CustomError) {
      ...
      if (ws.readyState === WebSocket.OPEN) ws.send(JSON.stringify({ action, success: false, msg: error.clientMessage }));
    } else if (
      ...
      if (ws.readyState === WebSocket.OPEN) ws.send(JSON.stringify({ action, success: false, msg: BAD_MESSAGE }));
      ...
      // any other Error
      console.error(`Action: ${action}\nError: ${error.message}`);
      if (ws.readyState === WebSocket.OPEN) ws.send(JSON.stringify({ action, success: false, msg: SERVER_ERROR }));
    } else {
