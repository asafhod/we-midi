...
const wsErrorHandler = (error: unknown, ws: WebSocket, action: unknown, data: any) => {
  try {
    if (error instanceof CustomError) {
      ...
      console.error(`Action: ${action} ${error.constructor.name}: ${error.message}`);
      ...
      console.error(
        `Action: ${action} MongoDB Duplicate Value Error: Batch operation aborted - Cannot insert duplicate value\nFor Object: ${JSON.stringify(
          invalidDocument
        )}`
      );
      ...
      console.error(
        `Action: ${action} MongoDB Duplicate Value Error: Cannot insert duplicate value - Field: ${
      ...
      console.error(`Action: ${action} MongoDB Validation Error: ${error.message}`);
      if (ws.readyState === WebSocket.OPEN) ws.send(JSON.stringify({ action, success: false, msg: BAD_MESSAGE, data }));
      ...
      console.error(`Action: ${action} Error Value: ${error}`);
      if (ws.readyState === WebSocket.OPEN) ws.send(JSON.stringify({ action, success: false, msg: SERVER_ERROR, data }));
    }
  } catch (error) {
    // Error message failed to send due to WebSocket connection error. Close the connection.
    ws.close(1011, SERVER_ERROR);
    console.error(`Action: ${action} Error: Error message failed to send to the WebSocket connection, so it has been closed.`);
  }
};
